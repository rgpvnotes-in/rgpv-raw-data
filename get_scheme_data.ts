...
type SchemeBucket = { name: string; id: number; pdfs: SchemeSyllabusRow[] };
type ProgramWithSchemes = { name: string; id: number; schemes: SchemeBucket[] };
type SchemeTypeData = { type: string; id: number; programs: ProgramWithSchemes[] };
...
const schemeSyllabusDataList = async (): Promise<SchemeTypeData[] | undefined> => {
  try {
    ...
    const schemeSyllabusFinalList: SchemeTypeData[] = [];
    const type = { name: "Scheme", id: 1 };
    const schemeSyllabusTypeData: SchemeTypeData = { type: type.name, id: type.id, programs: [] };
...
const writeData = async (schemeSyllabusFinalData: ProgramWithSchemes[]): Promise<void> => {
  try {
...
void withRetry("compileSchemeData", mainData, { retries: 3, delayMs: 1500 });
