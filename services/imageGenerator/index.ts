...
const imageTextFontSize = (postCaption: string): string => {
...
const cssForImage = (postCaption: string): string => `.box {
...
const htmlForImage = (postCaption: string): string => `<div class='box'>${postCaption}<br><br><br>
<span class='footer'>
...
export const postImageUrl = async (postCaption: string): Promise<string> => {
  try {
    ...
    if (imageGeneratorCredentials.username && imageGeneratorCredentials.password) {
    ...
    console.log(" something went wrong with image generation, returning default post image");
